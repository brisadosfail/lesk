...
curl -s -o /dev/null -X POST "https://api.cloudflare.com/client/v4/zones/$CloudflareZoneID/dns_records" \
     "${CF_AUTH_HEADERS[@]}" \
     --data '{ "type": "TXT", "name": "_dmarc.'$ServerName'", "content": "v=DMARC1; p=quarantine; sp=quarantine; rua=mailto:dmark@'$ServerName'; rf=afrf; fo=0:1:d:s; ri=86400; adkim=r; aspf=r", "ttl": 60, "proxied": false }'
  ...
  for (const toAddress of to) {
    try {
      let message = {
        ...
        list: {
          unsubscribe: [{
            url: "https://" + "'$ServerName'" + "?action=unsubscribe&u=" + toAddress,
            comment: "Cancelar Inscricao"
          }]
        }
      }
      ...
      results.push({ to: toAddress, status: "sent", messageId: result.messageId })
    } catch (err) {
      results.push({ to: toAddress, status: "error", error: err.message })
    }
  }
